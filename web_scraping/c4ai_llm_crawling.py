...
def ask_ai_for_relevant_urls(urls, query, model="deepseek-chat", provider="deepseek", api_key=os.getenv("DEEPSEEK_API_KEY")):
    """
    Uses LiteLLM to ask an LLM which URLs are likely to contain info about the query.
    Returns a list of plausible URLs.
    """
    if api_key:
        litellm.set_api_key(api_key)
    prompt = (
        f"Given the following list of URLs, which ones are most likely to contain information about '{query}'?\n"
    ...
    plausible_urls = [line.strip() for line in response['choices'][0]['message']['content'].splitlines() if line.strip().startswith("http")]
    return plausible_urls

# Example usage:
# urls = ["https://example.com/about", "https://example.com/formation", ...]
# plausible = ask_ai_for_relevant_urls(urls, "formations et programmes de formation", model="deepseek-chat", provider="deepseek", api_key="sk-...")
# print(plausible)
...
async def basic_deep_crawl():
    ...
    relevance_filter = ContentRelevanceFilter(
        query="formations et programmes de formation",
    ...
    downloads_path = os.path.join(os.getcwd(), "c4ai_downloads")  # Custom download path
    browser_config = BrowserConfig(accept_downloads=True, downloads_path=downloads_path)  # Enable downloads globally
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        output_dir = "c4ai_results_html"
        for idx, result in enumerate(results):
            filename = os.path.join(output_dir, f"result_{url_to_filename(result.url)}.html")
            # print("writing ",result.html[:100]," ...")  # Show first 100 characters of cleaned HTML
